...
class _SongActionSheetState extends ConsumerState<SongActionSheet> {
              ...
              return SafeArea(
                child: Column(
                  ...
                  children: [
                    ...
                    // 字体颜色
                    _buildFontColorRow(ctx2, c2, () => setSheetState(() {})),
                    const SizedBox(height: 16),
                  ],
                ),
              );
  ...
  Widget _buildFontColorRow(
    BuildContext ctx,
    ProviderContainer container, [
    VoidCallback? onSheetClosed,
  ]) {
    final colorHex = container.read(lyricFontColorProvider);
    final color = colorHex.isNotEmpty
        ? Color(int.parse(colorHex, radix: 16))
        : null;
    return InkWell(
      onTap: () {
        _showFontColorPicker(ctx, container).then((_) => onSheetClosed?.call());
      },
      child: Padding(
        padding: const EdgeInsets.symmetric(
          horizontal: AppSpacing.lg,
  ...
  Future<void> _showFontColorPicker(
    BuildContext ctx,
    ProviderContainer container,
  ) {
    final presetColors = [
    ...
    return showModalBottomSheet(
      ...
      builder: (ctx2) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            final currentHex = ProviderScope.containerOf(
              context,
            ).read(lyricFontColorProvider);
            return Container(
              decoration: const BoxDecoration(
                color: AppColors.surface,
                borderRadius: BorderRadius.vertical(top: Radius.circular(16)),
                  ...
                  children: [
                    ...
                    ListTile(
                      ...
                      title: Text(
                        '跟随主题/封面',
                        style: TextStyle(
                          color: currentHex.isEmpty
                              ? AppColors.primary
                              : AppColors.textPrimary,
                        ),
                      ),
                      trailing: currentHex.isEmpty
                          ? const Icon(
                              Icons.check,
                              size: 18,
                              color: AppColors.primary,
                          ...
                          : null,
                      onTap: () async {
                        c2.read(lyricFontColorProvider.notifier).state = '';
                        await _persistStringSetting(
                          container,
                          lyricFontColorProvider,
                          '',
                          (s) => s.setLyricFontColor(''),
                        );
                        if (ctx2.mounted) {
                          setState(() {});
                          Navigator.pop(ctx2);
